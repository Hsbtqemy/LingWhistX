...
fn emit_waveform_progress(app: &AppHandle, event: &WaveformProgressEvent) {
    let _ = app.emit("waveform-progress", event);
}

fn emit_waveform_ready(app: &AppHandle, event: &WaveformReadyEvent) {
    let _ = app.emit("waveform-ready", event);
}

fn emit_waveform_error(app: &AppHandle, event: &WaveformErrorEvent) {
    let _ = app.emit("waveform-error", event);
}

fn emit_waveform_cancelled(app: &AppHandle, event: &WaveformCancelledEvent) {
...
mod tests {
    ...
    fn smoke_mock_edit_export_flow() -> Result<(), String> {
        ...
        let txt_content = fs::read_to_string(txt_path)
            .map_err(|err| format!("Unable to read TXT export: {err}"))?;
